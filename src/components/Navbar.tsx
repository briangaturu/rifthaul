...
export default function Navbar() {
  ...
  const getDashboardPath = () => {
    switch (userType) {
      ...
      case 'transporter': return '/dashboard/transporter'
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          {isLoggedIn ? (
            <>
              <Link
                to={getDashboardPath()}
                className="text-[#F5F0E8] hover:text-white text-sm font-medium transition-colors"
              >
                Dashboard
              </Link>
              <button
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="flex flex-col gap-4 px-4 sm:px-6 py-6 bg-[#0D1117]/95 border-t border-white/5">
          <Link
            ...
            className={`text-sm font-medium transition-colors py-2 ${isActive('/') ? 'text-[#E8830A]' : 'text-[#8A95A3] hover:text-[#F5F0E8]'}`}
          >
            Home
          </Link>
          <Link
            to="/marketplace"
            onClick={() => setIsMenuOpen(false)}
            className={`text-sm font-medium transition-colors py-2 ${isActive('/marketplace') ? 'text-[#E8830A]' : 'text-[#8A95A3] hover:text-[#F5F0E8]'}`}
          >
            Marketplace
          </Link>
          <Link
            to="/about"
            onClick={() => setIsMenuOpen(false)}
            className={`text-sm font-medium transition-colors py-2 ${isActive('/about') ? 'text-[#E8830A]' : 'text-[#8A95A3] hover:text-[#F5F0E8]'}`}
          >
            About
          </Link>
          <Link
            to="/contact"
            onClick={() => setIsMenuOpen(false)}
            className={`text-sm font-medium transition-colors py-2 ${isActive('/contact') ? 'text-[#E8830A]' : 'text-[#8A95A3] hover:text-[#F5F0E8]'}`}
          >
            Contact
          </Link>
          ...
          {isLoggedIn ? (
            <>
              <Link
                to={getDashboardPath()}
                onClick={() => setIsMenuOpen(false)}
                className="text-sm font-medium text-[#F5F0E8] hover:text-white transition-colors py-2"
              >
                Dashboard
              </Link>
              <button
            ...
            <>
              <Link
                to="/login"
                onClick={() => setIsMenuOpen(false)}
                className={`text-sm font-medium transition-colors py-2 ${isActive('/login') || isActive('/register') ? 'text-[#E8830A]' : 'text-[#F5F0E8] hover:text-white'}`}
              >
                Log in
              </Link>
              <Link
