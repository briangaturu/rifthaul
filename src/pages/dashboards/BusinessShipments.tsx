...
import { useGetMyShipmentsQuery, useDeleteShipmentMutation } from '../../features/api/shipmentApi'
      ...
      <div className="p-4 md:p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[#8A95A3] text-sm">Track and manage all your posted shipments.</p>
          </div>
          <button
        ...
        <div className="bg-[#1C2128] border border-white/5 rounded-xl overflow-hidden">
          {isLoading && (
            <div className="p-12 text-center text-[#8A95A3] text-sm">Loading your shipments...</div>
          )}

          {isError && (
            <div className="p-12 text-center text-red-400 text-sm">
              Failed to load shipments. Make sure you are logged in.
            </div>
          )}
              ...
              <table className="w-full">
                <thead className="bg-[#0D1117] border-b border-white/5">
                  <tr>
                    ...
                    <th className="text-left text-[#8A95A3] text-xs font-semibold uppercase px-6 py-4">Budget</th>
                    <th className="text-left text-[#8A95A3] text-xs font-semibold uppercase px-6 py-4">Status</th>
                    <th className="text-left text-[#8A95A3] text-xs font-semibold uppercase px-6 py-4">Pickup</th>
                  ...
                  {(shipments ?? []).length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-6 py-16 text-center">
                        ...
                        <p className="font-display text-lg font-bold text-white mb-1">No shipments yet</p>
                        <p className="text-[#8A95A3] text-sm mb-4">Post your first shipment to get started</p>
                        <button
                          onClick={() => navigate('/dashboard/business/new-shipment')}
                          className="bg-[#E8830A] hover:bg-[#F5A030] text-[#111418] font-bold text-sm px-5 py-2.5 rounded-lg transition-all"
                        >
                          Post Shipment
                        </button>
                      </td>
                    ...
                    (shipments ?? []).map((shipment: any) => (
                      <tr key={shipment.shipmentId} className="border-b border-white/5 hover:bg-[#2D3748] transition-colors">
                        ...
                        <td className="px-6 py-4 text-[#E8830A] text-sm font-semibold">{formatBudget(shipment.budget)}</td>
